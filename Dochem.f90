      ...
      integer, intent(in) :: N
      integer :: ISS4SL, i, j, LO2, JT1
      real*8 :: AQ, BQ ,CQ, DLS4
      real*8 :: TAUO2, TAUCH4, TAUSO2
      real*8 :: FCO, XLJ
      real*8, dimension(nsp2,nz) :: D ! need to pass in nz and nsp2? I don't think so
      integer :: changeL
...
! c         if(j.eq.1)print *, i, ISPEC(I),' particle densities'
        ...
        if (ISPEC(I).EQ.'CO') FCO = usol(i,1)
        if (ISPEC(I).EQ.'O2') LO2 = i
      enddo
...
!now do the last 4 inert species that are the same in both codes
      do J=1,NZ
        ...
        if (CO2_inert.eq.1) D(NSP-1) = FCO2*Den(j) !If CO2 is inert, then put it in D
        if (N2_inert.eq.1) D(NSP) = (1. - USOL(LO2,J) - FCO2 - FCO)* DEN(J) !if N2 is inert, then it is rest of atmosphere.
...
! C
      ...
      ISS4SL=0  !checking if S4 is the the short-lived loop
...
! c-mab: (Basing this one on FH2-based distinction...) !mc - could we use the PLANET variable here?
      ...
      DO J=1,JTROP
        ...
        SCALE = RAIN(J)/RAIN(1) ! from rainout

        ZAP = ZAPNO * SCALE ! from lightning
...
! orig      DO 9 I=NQ1,NSP
   ...
   9  SL(I,J) = D(I,J)
      ...
      ! NW - this is where we can stop I think.
