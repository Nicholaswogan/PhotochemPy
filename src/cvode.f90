...
subroutine cvode(T0, usol_start, nnq, nnz, t_eval, num_t_eval, num_sol, rtol, atol, &
                 use_fast_jacobian, outfilename, solution, success)
  implicit none
  ...
  integer, intent(in) :: num_sol ! dimension of solution
  ...
  ! output
  double precision, dimension(num_sol,nnq,nnz),intent(out) :: solution
  ...
  do ii=1,num_t_eval
    ...
    if (outfilename .ne. "None") then
      ...
      enddo
      close(1)
      solution(1,:,:) = solution_temp
    else
      solution(ii,:,:) = solution_temp
    endif
...
subroutine cvode_equilibrium(rtol, atol, use_fast_jacobian, success)
  ...
  call cvode(T0, usol_init, nq, nz, t_eval, num_t_eval, num_sol, rtol, atol, &
             use_fast_jacobian, "None", solution, success)
