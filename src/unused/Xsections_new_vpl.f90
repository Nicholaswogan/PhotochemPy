      SUBROUTINE XS(species,nw,wavl,wav,T,DEN,j,columndepth,zy,IO2,lgrid)
...
!----------------------------Comments Correspond to line below---------------------------!
      ...
      if(lgrid .eq. 2) then ! APL very high resolution grid with updated xsec, mostly into FUV (resolves SED bands better)
         IF(species.eq.'CO2     ') CALL XS_CO2_alinc(nw,wavl,j)
         IF(species.eq.'HNO3    ') CALL XS_HNO3_alinc(nw,wavl,T,j)
         IF(species.eq.'NO2     ') CALL XS_NO2_alinc(nw,wavl,j)
         IF(species.eq.'H2O2    ') CALL XS_H2O2_alinc(nw,wavl,T,j)
         IF(species.eq.'H2CO    ') CALL XS_H2CO_alinc(nw,wavl,wav,j)
         IF(species.eq.'O3      ') CALL XS_O3_alinc(nw,wavl,T,j)
         IF(species.eq.'N2O     ') CALL XS_N2O_alinc(nw,wavl,wav,T,j)
         IF(species.eq.'CH3OOH  ') CALL XS_alinc(species,nw,wavl,j)
         IF(species.eq.'N2O5    ') CALL XS_N2O5_alinc(nw,wavl,j)
         IF(species.eq.'O2      ') CALL XS_O2_alinc(nw,wavl,j)
         IF(species.eq.'NO      ') CALL XS_NO_alinc(nw,wavl,j)
         IF(species.eq.'OCS     ') CALL XS_alinc(species,nw,wavl,j)
         IF(species.eq.'SO2     ') CALL XS_SO2_alinc(nw,wavl,j)
         IF(species.eq.'SO3     ') CALL XS_alinc(species,nw,wavl,j)
         IF(species.eq.'H2S     ') CALL XS_alinc(species,nw,wavl,j)
         IF(species.eq.'S4      ') CALL XS_S4(nw,wavl,j)
         IF(species.eq.'S3      ') CALL XS_S3(nw,wavl,j)


      else ! band model
        !returns CO+O to j and CO+O1D j+3 !ACK hardcoded needs fixing..
         IF(species.eq.'CO2     ') CALL XS_CO2(nw,wavl,j)
         IF(species.eq.'HNO3    ') CALL XS_HNO3(nw,wavl,T,j)
         IF(species.eq.'NO2     ') CALL XS_NO2(nw,wavl,T,j)
         IF(species.eq.'H2O2    ') CALL XS_H2O2(nw,wavl,wav,T,j)
         !returns H2, then HCO
         IF(species.eq.'H2CO    ') CALL XS_H2CO(nw,wavl,j)
         IF(species.eq.'O3      ') CALL XS_O3(nw,wavl,T,j)
         !returns N2 + O1D
         IF(species.eq.'N2O     ') CALL XS_N2O(nw,wavl,wav,T,j)
         IF(species.eq.'CH3OOH  ') CALL XS_CH3OOH(nw,wavl,j)
         !returns NO3 + NO2, then NO3 + NO + O  (temperature dependent!)
         IF(species.eq.'N2O5    ') CALL XS_N2O5(nw,wavl,wav,T,j)
         ...
         IF(species.eq.'SO2     ') CALL XS_SO2(nw,wavl,j)
         IF(species.eq.'SO3     ') CALL XS_SO3(nw,wavl,j)
         IF(species.eq.'H2S     ') CALL XS_H2S(nw,wavl,j)
      ...
      if(species .eq. 'HNO2    ') call xs_alinc(species,nw,wavl,j)
      IF(species.eq.'HO2     ') CALL XS_HO2(nw,wavl,wav,j,Ja)
      !T-dependent (not in use)

      !HAVEN"T INTEGRATED GRACES CODE
      IF(species.eq.'H2O     ') CALL XS_H2O(nw,wavl,wav,j)
      !returns O2 + O(1D), then O2 + O(3P)
      IF(species.eq.'CH4     ') CALL XS_CH4(nw,wavl,j)
      !returns  2 (3)CH2 +H2 then CH4 + (1)CH2
      IF(species.eq.'C2H6    ') CALL XS_C2H6(nw,wavl,j)
      !2 channels: NO + O2, NO2 + O
      !note NO3 is temperature dependent so would need to change if it does...
      IF(species.eq.'NO3     ') CALL XS_NO3(nw,wavl,T,j)
      !returns OH + CL
      IF(species.eq.'HOCL    ') CALL XS_HOCL(nw,wavl,j)
      !returns CL + CL (temp dependent)
      IF(species.eq.'CL2     ') CALL XS_CL2(nw,wavl,T,j)
      !returns CLO + O
      IF(species.eq.'CLOO    ') CALL XS_CLOO(nw,wavl,j)
      !returns CLO + O
      IF(species.eq.'OCLO    ') CALL XS_OCLO(nw,wavl,j)
      !returns CL + NO2
      IF(species.eq.'CLONO   ') CALL XS_CLONO(nw,wavl,j)
      !returns CL + NO
      IF(species.eq.'CLNO    ') CALL XS_CLNO(nw,wavl,j)
      !returns CL + NO2
      IF(species.eq.'CLNO2   ') CALL XS_CLNO2(nw,wavl,j)
      !returns HCO + CL (JPL calls this COHCL)  !products assumed from Yuk's model
      IF(species.eq.'CHCLO   ') CALL XS_CHCLO(nw,wavl,j)
      !returns CL + CH3  (temperature dependent!)
      !there is a far UV branch to H + CH2CL , but no JPL recomended cross section so ignored
      IF(species.eq.'CH3CL   ') CALL XS_CH3CL(nw,wavl,wav,T,j)
      !returns CL + CCL3  (temperature dependent!)
      IF(species.eq.'CCL4    ') CALL XS_CCL4(nw,wavl,wav,T,j)
      !returns CL + CL + CO
      IF(species.eq.'COCL2   ') CALL XS_COCL2(nw,wavl,j)
      !returns CH3O2 + NO2
      IF(species.eq.'CH3O2NO2') CALL XS_CH3O2NO2(nw,wavl,j)
      !returns CH3O + CL
      IF(species.eq.'CH3OCL  ') CALL XS_CH3OCL(nw,wavl,j)
      !returns CL + O1D and CL + O
      IF(species.eq.'CLO     ') CALL XS_CLO(nw,wavl,j)
      !returns CL + NO3, then CLO + NO2 (temperature dependent!)
      IF(species.eq.'CLONO2  ') CALL XS_CLONO2(nw,wavl,wav,T,j)
      !returns, HO2 + NO2, then OH + NO3
      IF(species.eq.'HO2NO2  ') CALL XS_HO2NO2(nw,wavl,j)
      !returns CL + CLOO, then CLO + CLO
      IF(species.eq.'CL2O2   ') CALL XS_CL2O2(nw,wavl,j)
      !returns CL + CLO
      IF(species.eq.'CL2O    ') CALL XS_CL2O(nw,wavl,j)
      !returns CLO + O2
      IF(species.eq.'CLO3    ') CALL XS_CLO3(nw,wavl,j)
      !returns CLOO + OCLO
      IF(species.eq.'CL2O4   ') CALL XS_CL2O4(nw,wavl,j)
      !returns H + CL.
...
!below here there are calls to XS_simple.
      IF(species.eq.'C2H6S  ')CALL XS_simple(species,nw,wavl,j)
      IF(species.eq.'C2H6S2 ')CALL XS_simple(species,nw,wavl,j)

!corg species
      IF(species.eq.'C2H2    ') CALL XS_C2H2(nw,wavl,j)
      IF(species.eq.'C2H4    ') CALL XS_C2H4(nw,wavl,j,Ja)
      IF(species.eq.'CH      ') CALL XS_C2H4(nw,wavl,j,Jd) ! shawn used C2H4 rate here
      IF(species.eq.'C3H8    ') CALL XS_C3H8(nw,wavl,j)
      IF(species.eq.'CH2CO   ') CALL XS_simple(species,nw,wavl,j)
      IF(species.eq.'CH3CHO  ') CALL XS_CH3CHO(nw,wavl,j)  !shawn used C2H4 rate here with 0.57/0.02/0.05 quantum yields
      IF(species.eq.'C3H6    ') CALL XS_C3H6(nw,wavl,j)
      IF(species.eq.'C2H5CHO ') CALL XS_simple(species,nw,wavl,j)
      IF(species.eq.'C3H3    ') CALL XS_simple(species,nw,wavl,j)    !shawn used C2H4 rate here (check this xsection)
      IF(species.eq.'CH3C2H  ') CALL XS_CH3C2H(nw,wavl,j)
      IF(species.eq.'CH2CCH2 ') CALL XS_CH2CCH2(nw,wavl,j)

!      added by nick
      IF(species.eq.'NH3     ') CALL XS_NH3(nw,wavl,j)
      IF(species.eq.'HCN     ') CALL XS_HCN(nw,wavl,j)
      IF(species.eq.'N2H4    ') CALL XS_N2H4(nw,wavl,j)

      IF(species.eq.'HNCO    ') CALL XS_HNCO(nw,wavl,j)
!      IF(species.eq.'NH      ') CALL XS_NH(nw,wavl,j)
!      IF(species.eq.'NH2X    ') CALL XS_NH2X(nw,wavl,j)
!      IF(species.eq.'NH2     ') CALL XS_NH2(nw,wavl,j)
!      IF(species.eq.'NCO     ') CALL XS_NCO(nw,wavl,j)

! these are just here to remind us that these XS files exist
!       CALL XS_CL2O3(nw,wavl,wav,T,DEN,JCL2O3)  !returns CLO + CLOO  (branch to CLO3??)
...
!       CALL XS_CHOCHO(nw,wavl,wav,T,DEN,100,101,102)  !ACK - hardcoded "J-numbers"
      ...
      SUBROUTINE XS_HNO3_alinc(nw,wl,tlev,jn)
!-----------------------------------------------------------------------------*
!=  PURPOSE:                                                                 =*
...
!=       HNO3 + HV -> HNO2 + O(1D)
      ...
      OPEN(UNIT=kin, &
           file=trim(rootdir)//'DATA/XSECTIONS/HNO3/HNO3_alinc.dat', &
           STATUS='old')
...
!     interpolate gridpoints if needed
...
!     interpolate temp data if needed
...
!     Main wl loop, with temperature dependence
!     Using Hu et al 2012's choice of QY cutoffs; there is significant variation in measurements
      ...
      do L=1,nw
         ...
         if(wl(l) .ge. 2000.) then
            ...
            qy(2,l) = 0.0
            qy(3,l) = 0.0
      !         else if(wl(l) .ge. 2500.) then
      !            qy(1,l) = 1.0 - 1.e-2*(3000. - wl(l))
      !            qy(2,l) = 1.0 - qy(1,l)
      !            qy(3,l) = 0.0
      !         else if(wl(l) .ge. 2220.) then
      !            qy(1,l) = 0.95 - 0.05/290. * (2500. - wl(l))
      !            qy(2,l) = 0.05 + 0.01/290. * (2500. - wl(l))
      !            qy(3,l) = 1.00 - qy(1,l) - qy(2,l)
         else if(wl(l) .ge. 1930.) then
      !            qy(1,l) = 0.9 - 0.4/300. * (2220. - wl(l))
      !            qy(2,l) = 0.06 + 0.24/300. * (2220. - wl(l))
      !            qy(3,l) = 1.00 - qy(1,l) - qy(2,l)
      ...
      !     set cross sections with temperature dependence
...
!      REAL*8 airlev(kz) ! currently this isn't used until declared again below
...
!*************** HNO3 photodissociation
! option 1 -  Burkholder et al. 1993
! option 2 -  Kevin's photo.dat
...
! HNO3 cross section parameters from Burkholder et al. 1993
      kin = 644
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/HNO3/HNO3_burk.abs',&
     &  STATUS='old')
      DO i = 1, 6
         READ(kin,*)
      ENDDO
...
! note different behavior in addpnt below than above
...
!      REAL*8 tlev(nz)  ! EWS - not used
!      REAL*8 airlev(nz) ! - EWS - not used
...
! option 1) cross sections from JPL97 recommendation (identical to 94 recommendation)+ quantum yield
! option 2) from Kevin no qy
      ...
      kin = 645
      OPEN(unit=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/HO2/HO2_jpl94.abs',&
     &  STATUS='OLD')
      READ(kin,*) idum, n
...
!      print *, yg1
      ...
      SUBROUTINE XS_NO2_alinc(nw,wl,jn)
!*-----------------------------------------------------------------------------*
!*=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/NO2/NO2_alinc.dat', &
          STATUS='old')
...
!     Main wl loop
!     quantum yields < 244 nm are 0.55 +-0.03 (Sun, Glass, & Curl 2001) or 0.51 +- 0.04 (Hancock & Morrison, 2011); at 193 nm
...
!     OPEN(UNIT=kin,
!    &  file=PHRT(1:INDEX(PHRT,' ')-1)//'DATAE1/NO2/NO2_ncar_00.abs',
!    &  STATUS='old')
!     n = 750
!     DO i = 1, n
...
! JPL 97 recommendation is identical
      ...
      kin = 647
         OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO2/NO2_jpl94.abs',&
     &  STATUS='old')
         READ(kin,*) idum, n
         DO i = 1, idum-2
            READ(kin,*)
...
! yg1, yg2 are per nm, so rescale by bin widths
      ...
      kin = 648
         OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO2/NO2_Har.abs',&
     &  status='old')
         DO i = 1, 9
            READ(kin,*)
         ENDDO
...
! from Gardiner, Sperry, and Calvert
      kin = 39
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO2/NO2_calvert.yld',&
     &  STATUS='old')
      DO i = 1, 8
         READ(kin,*)
      ENDDO
...
! - qy needs to be called yg1
      kin = 567
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO2/NO2qy_zahnle.yld',&
     &  STATUS='old')
...
!   CALCULATE NO2 QUANTUM YIELDS
!   from Kevin's original code...

!      DO 47 L=47,60
!      WAVN = WAV(L)/10.     !wav(L) is centered wavelength...
!  47  QYNO2(L) = 1. - 8.E-4*(WAVN - 275.)
!
...
!      print *, jn,photolabel
      ...
      SUBROUTINE XS_H2O2_alinc(nw,wl,tlev,jn)
! *-----------------------------------------------------------------------------*
!*=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/H2O2/H2O2_alinc.dat', &
         STATUS='old')
...
! options
!     1)temperature-dependent JPL 1994/97
!     2)Kevin's photo.dat data
...
! tabulated data up to 260 nm
      kin = 568
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/H2O2/H2O2_jpl94.abs',&
     &  STATUS='old')
      READ(kin,*) idum,n
      DO i = 1, idum-2
         READ(kin,*)
...
! Quantum yield (from Kevin's code - no reference)
      qy=0.72
...
! Quantum yield (no reference nor research)
      qy=1.0
...
! Quantum yield (no reference nor research)
! scale is Kevin's factor to (and I quote...)
!   SCALE UP S2 CROSS SECTIONS TO OBTAIN CORRECT S2 LIFETIME UP HIGH
...
!     1)Kevin's photo.dat data
      ...
      OPEN(UNIT=kin, &
        file=trim(rootdir)//'DATA/XSECTIONS/S3/S3_alinc.dat', &
        STATUS='old')
...
! c     1)Kevin's photo.dat data
      ...
      if (option.eq.1) then  !MPI Mainz data
      OPEN(UNIT=kin, &
        file=trim(rootdir)//'DATA/XSECTIONS/S4/S4_alinc.dat', &
        STATUS='old')
...
! input
      INTEGER nw,jn
! orig      REAL*8 wl(nw), wc(nw)   !orig was kw, but don't see why this is needed once nw is defined by gridw.f
...
!cc  Kevin's original code
!        do L=1,68
!          SS8L(L) = MAX(SS8R(L),SS8L(L))
!        enddo
...
! options
!     1)Kevin's photo.dat data
!     2)MPI data, high res from 160-260 (not there is some temperature dependence here which is not included), along with far UV data not in photo.dat
...
! Quantum yield (no reference nor research)
      ...
      kin = 576
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/H2S/H2S_mpi.abs',&
     &  STATUS='old')
...
!      IMPLICIT NONE
!      INTEGER kin,kj,kw
!      PARAMETER(kin=33,kj=33,kw=250)    !kin - file unit#
...
!**************** H2O *************
! options
!     1)from Graces - combination of three (JPL 97 recs)
!     2) Kevin's photo.dat file
...
!         print*,"HJtest",HJtest
      ...
      if (option.eq.2) then
        kin = 578
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/H2O/H2O_zahnle.abs',&
     &  STATUS='old')
...
! Quantum yield (no reference nor research)
      ...
      if (option.eq.3) then !Ranjan et al. 2020 cross sections
        kin = 579
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/H2O/H2O_ranjan.XS.dat',&
     &  STATUS='old')
...
! Quantum yield (no reference nor research)
      ...
      SUBROUTINE XS_CO2_alinc(nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! *-----------------------------------------------------------------------------*
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/CO2/CO2_alinc.dat', &
          STATUS='old')
...
! options
!     1)Kevin's photo.dat data
!     2)MC temp-dependent merge to zahnle grid (should re-evaluate)
...
!         print*,"HJtest",HJtest
      ...
      kin = 581
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CO2/CO2_zahnle.abs',&
     &  STATUS='old')
      ...
      kin = 582
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CO2/CO2D_zahnle.abs',&
     &  STATUS='old')
...
!-mc
!-mc yosh2kz_disw vector from ~/dev/CO2/co2_xsection.pro - 195K cross section
!-mc data from parkinson/yoshino as binned to this wl grid.
!      DATA SCO2ASW/1.4E-19, 7.1E-19, 5.4E-19, 5.2E-19, 5.1E-19, 3.7E-19,
!     2  2.1E-19, 7.4E-20, 3.1E-20, 9.7E-21/



      kin = 583
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CO2/CO2_claire1.abs',&
     &  STATUS='old')
...
!      print *, jn, photolabel
      ...
      SUBROUTINE XS_H2CO_alinc(nw,wl,wc,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/H2CO/H2CO_alinc.dat', &
          STATUS='old')
...
!         print*,"HJtest",HJtest
      ...
      kin = 584
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/H2CO/H2CO_zahnle.abs',&
     &  STATUS='old')
...
! Quantum yields:  from Kevin's photo.dat file
...
!yg3 is quantum yield for H2
      ...
      SUBROUTINE XS_SO2_alinc(nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/SO2/SO2_alinc.dat', &
          STATUS='old')
...
!     INCLUDE 'PHOTOCHEM/INPUTFILES/parameters.inc'
      ...
      PARAMETER (deltax = 1.E-4,biggest=1.E+36, zero=0.0)
!     CHARACTER*11 photolabel
!     INCLUDE 'DATA/INCLUDE/PHOTABLOK.inc'
!     CHARACTER*8 ISPEC
!     INCLUDE 'DATA/INCLUDE/PBLOK.inc'
!     INCLUDE 'DATA/INCLUDE/DBLOK.inc'
!     SAVE/PBLOK/
...
! options
!     1)Kevin's photo.dat data (for all three)
! - note that these cross sections contain quantum yields...
! - JPL -06 recommends looking at Hudson and Kieffer - 1975 natural stratosphere of 1974 CIAP monograph...
! JPL -02 has some better comments - need quantum yields to do this carefully.
...
!         print*,"HJtest",HJtest
      ...
      kin = 585
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/SO2/SO2_zahnle.abs',&
     &  STATUS='old')
...
!  Interpolate the cross sections to the wavelength grid:
!- note these are overwritten by higher resolution data below if LGRID=1
...
!      OPEN(UNIT=kin,file=
!     & trim(rootdir)//'DATA/XSECTIONS/SO2/longward1.txt',STATUS='old')
!      n4 = 3025
!      endif

! longward2.txt is non-systematic + 2007data to 250nm + 2013data from 250-320
!      OPEN(UNIT=kin,file=
!     & trim(rootdir)//'DATA/XSECTIONS/SO2/longward2.txt',STATUS='old')
!      n4 = 4159
...
!assuming quantum yield of 1 for <220nm and 0 otherwise
      ...
      SUBROUTINE XS_O3_alinc(nw,wl,tlev,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
! *=  Provide product of (cross section) x (quantum yield) for O3 photolysis   =*
! *=         O3 + HV -> O2 + O(1D)                                             =*
...
! C  O3 + HV -> 3 O(3P)
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/O3/O3_alinc.dat', &
          STATUS='old')
...
! options
!     1)Kevin's photo.dat data - combined with far uv data from MPI
! - assuming no temperature dependence in the shortwave

      option=1

      if (option.eq.1) then  !Kevin's data

!$$$C below here is temp to bin the far uv data from MPI to our grid...
!$$$      OPEN(UNIT=kin,
!$$$     &  file=trim(rootdir)//'DATA/XSECTIONS/O3/O3_mpi_SW_Mason96.abs',
!$$$     &  STATUS='old')
!$$$c      DO i = 1, 2
!$$$c         READ(kin,*)
!$$$c      ENDDO
...
!$$$      CALL addpnt(x1,y1,kdata,n1,x1(n1)*(1.+deltax),zero)
!$$$      CALL addpnt(x1,y1,kdata,n1,            biggest,zero)
!$$$      CALL inter2(nw+1,wl,yg1,n1,x1,y1,0)   !inter2 - points to bins
!$$$
!$$$      do i=1,nw
!$$$         print *, wl(i),yg1(i)
!$$$      enddo
...
!         print*,"HJtest",HJtest
      kin = 4356
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/O3/O3_zahnle.abs',&
     &  STATUS='old')
...
! ok, the first thing kevin does is combine the two cross sections in the intermediate regime
...
!now, there is also a temperature-dependent quantum yield implemented in the longwave loop
      ...
      do L=1,nw
!   THIS SUBROUTINE CALCULATES TEMPERATURE COEFFICIENTS USED TO FIND
!   THE O(1D) QUANTUM YIELD IN O3 PHOTOLYSIS.  (SEE JPL, 1983.)
...
! options
!     1)Kevin's photo.dat data
!     2)JPL-06 recomendation (more or less the same as above, but over wider grid.
...
! Quantum yield (no reference nor research)
      ...
      kin = 583
      OPEN(UNIT=kin,&
     & file=trim(rootdir)//'DATA/XSECTIONS/HCL/HCL_JPL06.abs',STATUS='old')
...
! Quantum yield ( - there are some channel data for excited states of CL but whatever...)
      ...
      SUBROUTINE XS_O2_alinc(nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! C  O2 + HV -> O + O(3P)
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/O2/O2_alinc.dat', &
          STATUS='old')
...
! this option calculates the O2 cross section for the modern atmosphere using
! the Schumman-Runge corrections from Allen and Frederick 1982.  This cross section
! is completely overwritten in SUBROUTINES/Photo.f when IO2=1 (i.e most of the time).
!  This cross section changes based on pressure,temperature
! and zenith angle, and so needs to be recomputed each time if these change
! BUT - if I'm never going to use it, I shouldn't bother recomputing each time
! when I go to a time-dependent code - rather I should focus on what needs to change with
! the exponential sums...

! for starters, I'm not going to make the S-R corrections work for a variable grid, although SO2HZ will...
...
!      ENDDO
      ...
      kin = 590
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/O2/O2_zahnle.abs',&
     &  STATUS='old')
...
!      stop
      kin = 591
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/O2/AllenFrederick1982.coeff',&
     &  STATUS='old')
...
!           if (L.eq.1) print *, (CL(I),I=1,NZ)
!           print *, CL(80),AM**(-1.0*(10.**CL(80)))
...
!     compute Shumman-Runge correction for O2 cross section at each height over each wavelength
          do I=1,NZ
            C = 10.**CL(I)
            SD = SIG0(I,L) * AM**(-C)
            SRO2(I,L) = min(SD,e_19)  ! e_19 = 2e-19
!            if (L.eq.1) print *, SD,min(SD,e_19)
!            if (L.eq.1) print *, C,AM**(-C),AM
         enddo

       enddo  !end loop over wavelength
!       print *, ''
!            print *, (SRO2(I,1),I=1,NZ)

!           stop    !uncomment this and pritn CL() statement above to investigate L=14 wierdness..
...
!      DO i = 1, 10
!       print*,'(After Stuff) x2,wl,y2,yg2',x2(i),wl(i),y2(i),yg2(i)
!      ENDDO
!      DO i = 1, 34
!       print*,'(After Stuff) x1,wl,y1,yg1',x1(i),wl(i),y1(i),yg1(i)
!      ENDDO
...
!but use high resolution for the S-R bands...
      kin = 595
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/O2/Yoshino92.abs',&
     &  STATUS='old')
...
!         print*,"HJtest",HJtest
      ...
      kin = 597
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH4/CH4_zahnle.abs',&
     &  STATUS='old')
...
! Quantum yields and reactions depend on wavelength and presence of hydrocarbons
      ...
      DO iw = 1, nw
         if (wl(iw).le.1230) then  !ack hardcoded wavelength
...
! Quantum yield's depend on if there are higher order hydrocarbon and wavelength
      HCtest=0
      do i=1,nsp
...
!      print *, jn, photolabel
      ...
      SUBROUTINE XS_NO_alinc(nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
         file=trim(rootdir)//'DATA/XSECTIONS/NO/NO_alinc.dat', &
         STATUS='old')
...
! data arrays
      INTEGER kdata
      PARAMETER(kdata=kw)
!      INTEGER n1, n2 ! EWS - not used
!      REAL*8 x1(kdata), x2(kdata), x3(kdata) ! EWS - not used
!      REAL*8 y1(kdata), y2(kdata) ! EWS - not used
...
!   NO PREDISSOCIATION COEFFICIENTS (ALLEN AND FREDERICK, 1982)
!
      DATA ANO/-1.790868E+1, -1.924701E-1, -7.217717E-2, 5.648282E-2,&
     &  4.569175E-2, 8.353572E-3, 3*0.,&
     &  -1.654245E+1, 5.836899E-1, 3.449436E-1, 1.700653E-1,&
     &  -3.324717E-2, -4.952424E-2, 1.579306E-2, 1.835462E-2,&
     &  3.368125E-3/
!
      DATA BNO/7.836832E+3, -1.549880E+3, 1.148342E+2, -3.777754E+0,&
     &  4.655696E-2, 1.297581E+4, -2.582981E+3, 1.927709E+2,&
     &  -6.393008E+0, 7.949835E-2/
!
...
!
!          COEFFICIENTS FOR NITROUS OXIDE (NO)
        do L=1,2
...
!     1)JPL-06 data
      ...
      kin = 599
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO3/NO3.abs',&
     &  STATUS='old')
...
! Quantum yield (is temperature dependent)
      kin = 599
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/NO3/NO3QY.dat',&
     &  STATUS='old')
...
!test to see if I interpolate each group if the numbers sum to 1000.
      ...
      do l=1,nw
!         print *, wl(l),yg2(l),yg3(l),yg4(l)
!         print *, wl(l),yg5(l),yg6(l),yg7(l)
      enddo
...
!              if (iw .eq. 94) print *, qy1(iw,i),qy2(iw,i)
         ENDDO
      ENDDO
      endif  !end option 1
...
!      write(14,*) jn, photolabel
      ...
      SUBROUTINE XS_N2O_alinc(nw,wl,wc,tlev,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! * local
      ...
      INTEGER L
      REAL*8 lambda
      real*8 a0,a1,a2,a3,a4, a
      real*8 b0,b1,b2,b3, b

      ierr = 0

      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/N2O/N2O_alinc.dat', &
          STATUS='old')
      ...
      do L=1,nw
         ...
         IF (lambda .GE. 173. .AND. lambda .LE. 240.) THEN
            DO iz = 1, nz
               ...
               B = (MAX(194.,MIN(tlev(iz),302.))-300.)*EXP(B)
...
!just use formula below..
      kin = 600
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/N2O/N2O_JPL06.abs',&
     &  STATUS='old')
...
! cross section is temperature dependent - see JPL-06
...
!*** Ravishankara), so quantum yield of O(1D) is assumed to be unity
      ...
      DO iw = 1, nw
         ...
         IF (lambda .GE. 173. .AND. lambda .LE. 240.) THEN
           DO iz = 1, nz
             t = MAX(194.,MIN(tlev(iz),302.))    !320 or 302K ??? - 320 from graces...
             A = (((A4*lambda+A3)*lambda+A2)*lambda+A1)*lambda+A0
             B = (((B3*lambda+B2)*lambda+B1)*lambda+B0)
             B = (t-300.)*EXP(B)
...
!     1)JPL-06 data
      ...
      kin = 601
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLO/CLO_JPL06.abs',&
     &  STATUS='old')
...
! Quantum yield (1 for O1D < 265 nm then 1 for O longward
...
!     1)JPL-06 data
      ...
      kin = 601
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/HOCL/HOCL_JPL06.abs',&
     &  STATUS='old')
...
! Quantum yield (JPL-06 (i.e <2% O and >95% OH) are cited so just using 1 branch
!      (i.e. ignoring HOCL + HV -> HCL + O)
...
! using temperature dependent formula from JPL-06

! Quantum yield is 1 (jpl-06)
...
!     1)JPL-06 data
      ...
      kin = 602
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLOO/CLOO_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 604
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/OCLO/OCLO_JPL06.abs',&
     &  STATUS='old')
...
! Quantum yield is 1  (JPL-06)
! some uncertainty about temperature dependence of bands AND about the possibily
! of extra branches (CL + O2 and CLOO), but following rec and using 1 and 1 branch
...
!     1)JPL-06 data
      ...
      kin = 605
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLONO/CLONO_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      if (option.eq.1) then  !JPL-06
              kin = 606
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLONO2/CLONO2_JPL06.abs',&
     &  STATUS='old')
...
! compute T-dependent cross section
     ...
     &           yg3(iw)*(tlev(iz)-296)*(tlev(iz)-296))
            sq(jn,iz,iw) = qy1 * xsss
            sq(jn+1,iz,iw) = qy2 * xsss

!            if(iz .eq. 1) write(*,333) wc(iw), xs
! 333        format(0pf8.3,1pe11.4)
...
!     1)JPL-06 data
      ...
      if (option.eq.1) then  !JPL-06
              kin = 607
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLNO/CLNO_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 601
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLNO2/CLNO2_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 608
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CHCLO/CHCLO_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 610
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3CL/CH3CL_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 611
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CCL4/CCL4_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 612
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/COCL2/COCL2_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 data
      ...
      kin = 613
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CL2O2/CL2O2_JPL06.abs',&
     &  STATUS='old')
...
! Quantum yield (1 for CL < 390 nm then 0.9 longward
!NOTE - the second branch is uncertain - I should try to vary this when tuning
...
!     1)IUPAC 2006 data
      ...
      kin = 615
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3O2NO2/CH3O2NO2_IUPAC2006.abs',&
     &  STATUS='old')
...
! assuming Quantum yield is 1 without research
      qy=1.0
...
!     1)IUPAC 2008 data
      ...
      kin = 616
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3OCL/CH3OCL_IUPAC2008.abs',&
     &  STATUS='old')
...
! assuming Quantum yield is 1 (other channel is negligble (plus we don't do CH2O anyway...)
      qy=1.0
...
!     1)JPL-06 rec
      ...
      kin = 616
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3OOH/CH3OOH_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 rec
      ...
      kin = 617
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/HO2NO2/HO2NO2_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06 rec
      ...
      kin = 618
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CL2O/CL2O_JPL06.abs',&
     &  STATUS='old')
...
! Quantum yield is 1, ignoring potential other branches at short wavelengths...
      qy=1.0
      ...
      SUBROUTINE XS_N2O5_alinc(nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/N2O5/N2O5_alinc.dat', &
          STATUS='old')
...
!     1)JPL-06 rec
      ...
      kin = 619
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/N2O5/N2O5_JPL06_298.abs',&
     &  STATUS='old')
...
! open/read in temperature-dependent parameter
      kin = 620
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/N2O5/N2O5td.abs',&
     &  STATUS='old')
...
! ok now have data read into yg1, and A and B factors interpolated on to the grid



!      do i=1,nw
!       print *, wl(i), yg1(i),A(i),B(i)
!      enddo

!      stop
! see N2O5thoughts.txt for details
! also test the sq's below.
! - make sure there are 0's in the proper place.
! - also check 420 nm.
...
! options
!     1)extraction of Kopitzky et al 2002 by Mark Claire
      ...
      kin = 621
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CLO3/CLO3_Kopitzky_MC.abs',&
     &  STATUS='old')
...
!     1)JPL-06
      ...
      kin = 620
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CL2O3/CL2O3_JPL06.abs',&
     &  STATUS='old')
...
!     1)JPL-06
      ...
      if (option.eq.1) then  !
              kin = 620
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CL2O4/CL2O4_JPL06.abs',&
     &  STATUS='old')
...
!     1)from MPI database
      ...
      kin = 622
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CS2/CS2_mpi.abs',&
     &  STATUS='old')
      ...
      kin = 629
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CS2/CS2X_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      if (option.eq.1) then  !MPI
              kin = 630
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3SH/CH3SH_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data file
     ...
     &  trim(species),'/',trim(species),'_mpi.abs'
      ...
      ! XsecFile = trim(rootdir)//'DATA/XSECTIONS/'//trim(species)&
...
! Count lines in input file, allocate size of x1 and y1 accordingly
      kdata=0
      DO i = 1, 100000
         READ(kin,*,end=10)
         kdata=kdata+1
...
! Quantum yield (this could be read in from the reactions.rx file, eventually)
! but for now, xs_simple is restricted to molecules with one channel.
      qy=1.00
...
!     1)JPL-06 rec
      ...
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CHOCHO/CHOCHO_hybrid.abs',&
     &  STATUS='old')
...
! Quantum yield depends on wavelength and is read in from a table in JPL-06
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CHOCHO/CHOCHO.qy',&
     &  STATUS='old')
...
! this is not finished, only used for interpolative purposes...
...
! options
!     1)MPI cross sections, Jim's branching ratios
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/C2H2/C2H2_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3CHO/CH3CHO_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/C3H6/C3H6_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH3C2H/CH3C2H_mpi.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/CH2CCH2/CH2CCH2_kasting.abs',&
     &  STATUS='old')
...
!     1)Kasting data
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/C2H4/C2H4_kasting.abs',&
     &  STATUS='old')
...
!     1)MPI data
      ...
      if (option.eq.1) then  !Kevin's data
      OPEN(UNIT=kin,&
     &  file=trim(rootdir)//'DATA/XSECTIONS/C3H8/C3H8_mpi.abs',&
     &  STATUS='old')
...
! Quantum yields done in wl loop. diff at ly a then everywhere else
...
!      print *, jn, photolabel
      ...
      SUBROUTINE XS_alinc(species,nw,wl,jn)
! *-----------------------------------------------------------------------------*
! *=  PURPOSE:                                                                 =*
! *=  Provide product of (cross section) x (quantum yield) for generic photo.  =*
! *=  Cross section:  MPI database format, read in units of nm and cm^-2       =*
...
! * local
      ...
      OPEN(UNIT=kin, &
          file=trim(rootdir)//'DATA/XSECTIONS/' // trim(species) // '/' // &
...
!     1)Data in Atmos folder
     ...
     &  STATUS='old')
      ...
      n1 =  223
      DO i = 1, n1
        READ(kin,*) x1(i), dummy(i), y1(i) ! Read in data, but don't use the middle column
      ENDDO
      CLOSE (kin)
...
!     1)Data in Atmos folder
     ...
     &  STATUS='old')
      ...
      n1 =  100
      DO i = 1, n1
        READ(kin,*) x1(i), y1(i) ! Read in data
      ENDDO
      CLOSE (kin)
