...
subroutine gridw(nw,wl,wc,wu,LGRID,err)
  !*-----------------------------------------------------------------------------*
  !*=  PURPOSE:                                                                 =*
  ...
  if (LGRID.eq.0) then
    ...
    err = 'LGRID=0 is the only option'
    return
  endif
...
SUBROUTINE readflux(flux_txt,nw,wl,f,err)
  ...
  ierr = 0
  kin = 797
  OPEN(UNIT=kin,file=flux_txt, STATUS='old',iostat=io)
  if (io /= 0) then
    err = 'Unable to open the solar flux file '//trim(flux_txt)
    return
  endif
  ...
  !so yg3 is flux on the model wavelength grid
   ...
   IF (ierr /= 0) THEN
     err = 'Problem reading solar flux file '//trim(flux_txt)
     return
   ENDIF
